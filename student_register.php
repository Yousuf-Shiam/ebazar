...
if(($_POST['username'] != NULL) && ($_POST['first_name'] != NULL) && ($_POST['last_name'] != NULL) && ($_POST['password'] != NULL) && ($_POST['confirm'] != NULL) && ($_POST['email'] != NULL) && ($_POST['password'] == $_POST['confirm'])){
    ...
    } elseif (!(str_ends_with($email, '.edu') or str_ends_with($email, '.ac.bd') or str_ends_with($email, '.edu.bd'))){
        echo "Provide a student email account for student version.";
    } else{
        $sql = "INSERT INTO users VALUES ('$username', '$first_name', '$last_name', '$passwordHash', '$email', 'student')";
